...
function query_buy_detail(opt, callback) {
  ...
  Promise.all([
    ...
    info.hard_limit = results[1][0].hard_limit;
...
function calculate_order_qty(extra_info, opt, cmp) {
  ...
  if (signal_status === 'PENDING_CLOSE') {
    ...
    const hard_limit = extra_info.hard_limit;
    amt_available = (hard_limit && hard_limit <= amt_available) ? hard_limit : amt_available;
