...
function query_buy_detail(opt, callback) {
  ...
  Promise.all([
    ...
    info.stock_budget = results[1][0].stock_budget;
    info.strategy_stock = results[1][0].data$(false);
    info.nrr = results[1][0].nrr;
    callback(null, info);
  });
}

function calculate_order_qty(extra_info, opt, cmp) {
  // calculate order_quantity
  // cmp = current market price / last trade price
  let order_qty = 0;

  const signal_status = opt.signal_status;
  if (signal_status === 'PENDING_CLOSE') {
    if (extra_info.strategy_stock.order_log) {
      order_qty = extra_info.strategy_stock.order_log.order_obj.quantity;
    }
  } else {
    ...
    // let amt_available = budget_avail * allowed_budget_for_stock * normalized_rate_of_return;
    const stock_budget = (extra_info.stock_budget || 1000); // this hard coded value shouldbe removed
    // amt_available = (stock_budget && stock_budget <= amt_available) ? stock_budget : amt_available;
    const allowed_qty = Math.round(stock_budget / cmp);
    order_qty = allowed_qty;
    logger.debug('allowed_qty:', allowed_qty, 'order_qty:', order_qty);
...
function buy(opt, cb) {
  ...
  query_more_buy_detail(opt).then((extra_info) => {
    const order_qty = calculate_order_qty(extra_info, opt, cmp);
    logger.debug('calculated qty:', order_qty);
